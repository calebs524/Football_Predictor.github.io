        ...
        .then(data => {
            ...
            lines.forEach(line => {
                ...
                if (line.startsWith('Image:')) {
                    ...
                    if (buffer !== '') {
                        sectionHtml += `<div class="section"><p>${buffer.trim()}</p></div>`;
                        buffer = '';
                    }
                    const tableId = line.replace('Table:', '').trim();
                    const tableHTML = document.getElementById(tableId);
                    if (tableHTML) {
                        sectionHtml += tableHTML.innerHTML;
                    } else {
                        sectionHtml += `<p style="color:red;">[Missing table ${tableId}]</p>`;
                    }
                }
                else {
                    if (insideText) {
